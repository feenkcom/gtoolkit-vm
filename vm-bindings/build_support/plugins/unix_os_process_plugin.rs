...
pub fn unix_os_process_plugin(core: &Core) -> Option<Plugin> {
    let file_plugin = file_plugin(core).expect("Requires file plugin");
    let socket_plugin = socket_plugin(core).expect("Requires socket plugin");
